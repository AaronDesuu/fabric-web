...
export async function signInWithEmail(email, password) {
    ...
    // We utilize the standard data/error return pattern from Supabase
    const { error: rpcError } = await supabase.rpc('update_admin_last_login');
    if (rpcError) {
        console.warn('Failed to update admin login stats:', rpcError);
    }
